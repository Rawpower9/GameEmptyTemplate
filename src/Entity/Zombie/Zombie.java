    ...
    public void move(Plant[][] plants){
        ...
        for (int w = 0; w < plants.length; w++) {
            for(int c = 0; c < plants[w].length; c++) {
                if (plants[w][c] != null) {
                    if (Math.sqrt(Math.pow(plants[w][c].getPosition().getXPixel() - this.getPosition().getXPixel(), 2)
                            + Math.pow(plants[w][c].getPosition().getYPixel() - this.getPosition().getYPixel(), 2))
                            <= this.sidelength / 2) {
